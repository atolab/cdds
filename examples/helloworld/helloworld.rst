...
that can be found examples/helloworld/bin directory. These two
executables are called HelloworldPublisher and
...
examples/helloworld directory. It consists out of the
...
A Linux native Makefile is provided in the examples/helloworld
directory. Just go to that directory and type :code:`make`.
...
data layer. The HelloWorld uses the idl language.

.. literalinclude:: HelloWorldData.idl
	:linenos:
	:language: idl
...
The tool dds_idlc (idl compiler) is supplied for this.

The compilation from idl into c source code is as simple as
calling idlc with the idl file.
::
