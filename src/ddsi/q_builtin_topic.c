...
static void generate_product_data (_Out_ DDS_ProductDataQosPolicy *a, _In_ const struct proxy_participant *proxypp)
{
...
void write_builtin_topic_proxy_participant (_In_ const struct proxy_participant *proxypp, _In_ nn_wctime_t timestamp)
{
  ...
  generate_key(&(data.key), &(proxypp->gid));
  generate_user_data(&(data.user_data), &(proxypp->plist->qos));
...
void write_builtin_topic_proxy_participant_cm (_In_ const struct proxy_participant *proxypp, _In_ nn_wctime_t timestamp)
{
  ...
  generate_key(&(data.key), &(proxypp->gid));
  generate_product_data(&(data.product), proxypp);
