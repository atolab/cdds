...
#ifdef _WRS_KERNEL
       taskDelay(1); /* force scheduling */
#endif
#if defined INTEGRITY || defined VXWORKS_RTP || defined __QNX__
       sched_yield(); /* force scheduling */
#endif
...
static void tc_os_mutexLock (void)
{
    ...
    os_procAttrInit (&mutex_os_procAttr);
  #ifdef _WRS_KERNEL
    mutex_os_procAttr.schedPriority=101;
    mutex_os_threadAttr.schedPriority=101;
  #endif
  #ifdef VXWORKS_RTP
    mutex_os_procAttr.schedPriority=101;
  #endif
  ...
  #endif
    CU_ASSERT (os_mutexLock_s (&sd->global_mutex));
    os_mutexUnlock (&sd->global_mutex);
...
static void tc_os_mutexTryLock (void)
{
  ...
  #ifdef VXWORKS_RTP
    printf ("N.A - Mutexs are recursive on VxWorks RTP so this test is  disabled\n");
  #endif
  #endif
    result = os_mutexTryLock (&sd->global_mutex);
