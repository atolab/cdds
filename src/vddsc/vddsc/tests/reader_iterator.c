...
reader_iterator_init(void)
{
    ...
    g_topic = dds_create_topic(g_participant, &Space_Type1_desc, create_topic_name("vddsc_read_iterator_test", name, sizeof name), NULL, NULL);
 ...
 *************************************************************************************************/
/*************************************************************************************************/
Test(vddsc_read_next, reader, .init=reader_iterator_init, .fini=reader_iterator_fini)
{
    dds_return_t cnt = 0;
    dds_return_t ret = 1;
...
/*************************************************************************************************/
TheoryDataPoints(vddsc_read_next, invalid_buffers) = {
        DataPoints(void**,             g_samples, g_loans, (void**)0),
        DataPoints(dds_sample_info_t*, g_info,    (dds_sample_info_t*)0   ),
};
Theory((void **buf, dds_sample_info_t *si), vddsc_read_next, invalid_buffers, .init=reader_iterator_init, .fini=reader_iterator_fini)
{
    dds_return_t ret;
 ...
 *************************************************************************************************/
/*************************************************************************************************/
Test(vddsc_read_next_wl, reader, .init=reader_iterator_init, .fini=reader_iterator_fini)
{
    dds_return_t cnt = 0;
    dds_return_t ret = 1;
...
/*************************************************************************************************/
TheoryDataPoints(vddsc_read_next_wl, invalid_buffers) = {
        DataPoints(void**,             g_loans, (void**)0),
        DataPoints(dds_sample_info_t*, g_info,    (dds_sample_info_t*)0   ),
};
Theory((void **buf, dds_sample_info_t *si), vddsc_read_next_wl, invalid_buffers, .init=reader_iterator_init, .fini=reader_iterator_fini)
{
    dds_return_t ret;
...
/*************************************************************************************************/
TheoryDataPoints(vddsc_take_next, invalid_buffers) = {
        DataPoints(void**,             g_samples, g_loans, (void**)0),
        DataPoints(dds_sample_info_t*, g_info,    (dds_sample_info_t*)0   ),
};
Theory((void **buf, dds_sample_info_t *si), vddsc_take_next, invalid_buffers, .init=reader_iterator_init, .fini=reader_iterator_fini)
{
    dds_return_t ret;
    cr_assume((buf != g_samples) || (si != g_info));
    cr_assume(buf != g_loans);
...
/*************************************************************************************************/
TheoryDataPoints(vddsc_take_next_wl, invalid_buffers) = {
        DataPoints(void**,             g_loans, (void**)0),
        DataPoints(dds_sample_info_t*, g_info,    (dds_sample_info_t*)0   ),
};
Theory((void **buf, dds_sample_info_t *si), vddsc_take_next_wl, invalid_buffers, .init=reader_iterator_init, .fini=reader_iterator_fini)
{
    dds_return_t ret;
    cr_assume((buf != g_loans) || (si != g_info));
