...
void delete_entities(void)
{
    ...
    //cr_assert_eq(dds_err_nr(result), DDS_RETCODE_OK, "Recursively delete entities, Expected(%s) Returned(%s)",
     ...
     //   dds_err_str(result));
    cr_assert_eq(dds_err_nr(result), DDS_RETCODE_OK, " returned %d : expected %d", dds_err_nr(result), dds_err_nr(DDS_RETCODE_OK));
...
Test(vddsc_reader, return_loan_bad_params, .init = create_entities, .fini = delete_entities)
{
    dds_return_t result;
    void **buf = NULL;
...
Test(vddsc_reader, return_loan_success, .init = create_entities, .fini = delete_entities)
{
    void **buf;
    void *buf2 = NULL;
    dds_return_t result;
